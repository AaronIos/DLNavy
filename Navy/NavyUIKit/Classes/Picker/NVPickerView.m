...
@property (nonatomic, copy) NSArray* arraySubDatas;
@property (nonatomic, assign) NSInteger relevanceIndex;
...
- (void) onDone:(id)sender {
    ...
    if (self.isRelevance){
        NSArray* arraySelectedTitles    = @[self.mainTitle,
                                            self.subTitle];
        self.doneBlock(arraySelectedTitles);
    }
    else {
        if (self.delegate &&
...
- (NSInteger) pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (self.isRelevance) {
        if ([[self.arrayPickerDataModel objectAtIndex:component] isKindOfClass:[NVPickerListModel class]]) {
            NVPickerListModel* listModel    = [self.arrayPickerDataModel objectAtIndex:component];
            return [listModel.items count];
        } else {
            NSArray* arrayDataSources        = [self.arrayPickerDataModel objectAtIndex:component];
            self.arraySubDatas               = [arrayDataSources objectAtIndex:self.relevanceIndex];
            return [self.arraySubDatas count];
        }
    } else {
        NVPickerListModel* listModel    = [self.arrayPickerDataModel objectAtIndex:component];
        return [listModel.items count];
    ...
    if (self.isRelevance) {
        if ([[self.arrayPickerDataModel objectAtIndex:component] isKindOfClass:[NVPickerListModel class]]) {
            ...
            attributedString = [[NSAttributedString alloc] initWithString:dataModel.title
                                                               attributes:ATTR_DICTIONARY(COLOR_HM_BLACK, self.fontSize)];
            self.mainTitle                   = [attributedString string];
            return attributedString;
        } else {
            
            NSString* string                 = [self.arraySubDatas objectAtIndex:row];
            NSAttributedString* attributedString = nil;
            attributedString = [[NSAttributedString alloc] initWithString:string
                                                               attributes:ATTR_DICTIONARY(COLOR_HM_BLACK, self.fontSize)];
            
            self.subTitle                     = [attributedString string];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    NSArray* arrayDataSources = nil;
    switch (component) {
        case 0:
        {
            self.relevanceIndex              = row;
            [self.uiPickerView reloadComponent:component + 1];
            [self.uiPickerView selectRow:component inComponent:component + 1 animated:YES];
        }
            break;
        case 1:
            arrayDataSources                = [self.arrayPickerDataModel objectAtIndex:component];
            self.arraySubDatas              = [arrayDataSources objectAtIndex:row];
            break;
        default:
            break;
    }
